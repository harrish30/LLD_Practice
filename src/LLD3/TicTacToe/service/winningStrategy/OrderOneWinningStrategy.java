...
public class OrderOneWinningStrategy implements WinningStrategy
{
    ...
    @Override
    public Player checkWinner(Board board, Move lastMove, int [] draw)
    {
        Player player = lastMove.getPlayer();
        char symbol = player.getSymbol();
        int row = lastMove.getCell().getRow();
        int col = lastMove.getCell().getCol();
        boolean winnerResult = ((checkCorner(row, col) && winnerCheckForCorners(board.getMatrix(), symbol, draw))
        || (checkLeftDiagonal(row, col) && checkAndUpdateForLeftDiagonalHashMap(symbol, draw))
        || (checkRightDiagonal(row, col) && checkAndUpdateForRightDiagonalHashMap(symbol, draw))
        || (checkAndUpdateForRowHashMap(row, symbol, draw))
        || (checkAndUpdateForColHashMap(col, symbol, draw)));
    ...
    private boolean checkAndUpdateForRowHashMap(int row, char symbol, int [] draw)
    {
        ...
        {
            int rowHashMapSize = rowHashMap.size();
            if(rowHashMapSize == 1)
            {
                //draw condition
                draw[0]--;
    ...
    //TODO: shorten this code using inbuilt hashmap methods
    private boolean checkAndUpdateForColHashMap(int col, char symbol, int [] draw)
    {
        ...
        {
            int colHashMapSize = colHashMap.size();
            if(colHashMapSize == 1)
            {
                //draw condition
                draw[0]--;
    ...
    //TODO: shorten this code using inbuilt hashmap methods
    private boolean checkAndUpdateForLeftDiagonalHashMap(char symbol, int [] draw)
    {
        ...
        {
            int leftDiagonalHashMapSize = leftDiagonal.size();
            if(leftDiagonalHashMapSize == 1)
            {
                //draw condition
                draw[0]--;
            }
    ...
    //TODO: shorten this code using inbuilt hashmap methods
    private boolean checkAndUpdateForRightDiagonalHashMap(char symbol, int [] draw)
    {
        ...
        {
            int rightDiagonalHashMapSize = rightDiagonal.size();
            if(rightDiagonalHashMapSize == 1)
            {
                //draw condition
                draw[0]--;
            }
    ...
    private boolean winnerCheckForCorners(List<List<Cell>> matrix, char symbol, int [] draw)
    {
        ...
        {
            int cornerHashMapSize = cornerHashMap.size();
            if(cornerHashMapSize == 1)
            {
                //draw condition
                draw[0]--;
            }
